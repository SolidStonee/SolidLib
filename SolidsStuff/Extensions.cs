...
namespace SolidLib
{
    ...
    public static class WalkieTalkieExtensions
    {
        public static void TransmitOneShotAudio(Transform audioSource, AudioClip clip, float vol = 1f, float pitch = 1f)
        {
            ...
            {
                if (WalkieTalkie.allWalkieTalkies[i].playerHeldBy == null ||
                    !WalkieTalkie.allWalkieTalkies[i].clientIsHoldingAndSpeakingIntoThis ||
                    !WalkieTalkie.allWalkieTalkies[i].isBeingUsed)
                {
                    continue;
                }

                float num = Vector3.Distance(WalkieTalkie.allWalkieTalkies[i].transform.position,
                ...
                for (int j = 0; j < WalkieTalkie.allWalkieTalkies.Count; j++)
                {
                    ...
                    {
                        float num2 = Mathf.Lerp(WalkieTalkie.allWalkieTalkies[i].maxVolume, 0f,
                            num / (WalkieTalkie.allWalkieTalkies[i].recordingRange + 3f));
